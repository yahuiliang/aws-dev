...
render_dev_box_setup() {
  ...
  local install_desktop install_cursor
  ...
  terraform -chdir="$tf_dir" console -no-color <<< \
    "templatefile(\"files/dev-box-setup.sh.tpl\", { dev_username = \"$(tfvar dev_username dev)\", install_docker = ${install_docker}, install_desktop = ${install_desktop}, install_cursor = ${install_cursor}, desktop_rdp_public = $(tfvar desktop_rdp_public false), dev_rdp_password_b64 = \"${rdp_pw_b64}\", ssh_public_key = chomp(file(\"${pub_path}\")), auto_stop_idle_minutes = $(tfvar auto_stop_idle_minutes 120), auto_stop_check_interval_minutes = $(tfvar auto_stop_check_interval_minutes 5), aws_region = \"$(tfvar aws_region us-west-2)\", auto_stop_script_b64 = ${auto_stop_b64}, block_ssh_until_ready = $(tfvar block_ssh_until_ready true) })" \
