...
ARGS="{ dev_username = \"dev\", install_docker = true, install_leetcode_cli = true, ssh_public_key = chomp(file(\"${KEY}\")), auto_stop_idle_minutes = 30, auto_stop_check_interval_minutes = 5, aws_region = \"us-west-2\", auto_stop_script_b64 = base64encode(file(\"files/auto-stop.sh\")), block_ssh_until_ready = true }"
